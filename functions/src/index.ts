...
export const createTable = onCall(async (req) => {
  const uid = uidOf(req);
  const { tier = "10/20", buyIn, name = "Player", bots = [], currency = "play", assisted = false, isPublic = true } = (req.data ?? {}) as
  ...
  const T = TIERS[tier] ?? TIERS["10/20"]!;
